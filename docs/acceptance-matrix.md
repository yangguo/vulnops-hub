...
| Exposure generation/candidate queue | Partial | Matching behavior exists; operator-facing candidate queue is not exposed. *2026-09-09: the orchestration gap is closed — the outbox orchestrator consumed a real `vulnops.sbom.processed.v1` event, queried the real api.osv.dev, and wrote 14 exposures to the `exposures` table (see [the staging evidence log](operations/integrated-staging.md)). Against the live OSV API the outcome is candidate-only: `/v1/querybatch` returns `{id, modified}` stubs without `affected` ranges (the single `POST /v1/query` does return full ranges), and version-qualified purls sent together with a `version` param are rejected with 400 — so deterministic matching and auto case creation remain demonstrated in fixtures only. Candidate review UI still open* |
