...
| SBOM match | Partial (target PASS in staging) | CycloneDX/SPDX parser tests; `tests/matching/test_purl_range_match.py::test_purl_in_osv_range_creates_deterministic_exposure`; `tests/workflows/test_exposure_orchestration.py::test_m2_e2e_purl_match_creates_one_host_bound_active_finding_and_replays_cleanly` (target-scoped `GHSA-6c3j-c64m-qhgq` / `CVE-2019-11358` assertions); *2026-09-13: authenticated host inventory plus host-bound `pkg:npm/jquery@3.3.9` made a real OSV request and persisted the target GHSA/CVE alias, PURL range, active exposure, case, and replay-safe SBOM identity — see [staging evidence](operations/integrated-staging.md)* | Shared adopter staging replay of the same drill; production-shaped evidence beyond local sandbox |
