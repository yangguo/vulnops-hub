...
> **Baseline:** `main` at `7591914c`, with local acceptance extension `24ef9b9c`
...
| CycloneDX/SPDX ingestion | Verified in sandbox | API, parser, hashing, persistence, and idempotency tests exist. *2026-09-08: real authenticated submission through the staging Keycloak/API path recorded; 2026-09-12: MinIO raw-object readback matched the stored SHA-256 under an explicitly authorized principal — see [the staging evidence log](operations/integrated-staging.md)* |
...
| Exposure generation/candidate queue | Partial | Matching behavior and orchestration exist; *2026-09-09: outbox orchestrator consumed a real `vulnops.sbom.processed.v1` event, queried the real api.osv.dev, and wrote 14 exposures to the `exposures` table; candidate review API and console shipped (`tests/api/test_exposure_review.py`, `CandidateReviewView.vue`)*. *2026-09-12: an explicitly simulated, scanner-confirmed upstream finding traversed the live queue and produced a confirmed exposure and case under replay.* Against the live OSV API the outcome is candidate-only: `/v1/querybatch` returns `{id, modified}` stubs without `affected` ranges (the single `POST /v1/query` does return full ranges), and version-qualified purls sent together with a `version` param are rejected with 400 — so deterministic OSV matching remains demonstrated in fixtures only |
