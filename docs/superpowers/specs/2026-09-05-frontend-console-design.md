...
日期：2026-09-05
状态：已实施（提交 `c48e75c` 至 `5bf2b88`）。本文保留原始设计决策；下一阶段认证设计见 `docs/security/oidc-rbac-design.md`。

## 1. 背景与目标

VulnOps Hub 后端技术预览（FastAPI 模块化单体）当时已实现 SBOM 摄取与整改工单
（case）生命周期，但尚无前端界面，日常运营（triage、分派、SLA 跟踪、
