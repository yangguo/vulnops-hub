...
| `raw-evidence-demo` | org-demo | auditor + `evidence:raw:read` permission |
...
and one linked case for `GHSA-6c3j-c64m-qhgq`; the fixed occurrence produced
zero **active** exposures for that GHSA (range evaluation is `not_affected` at
`3.4.0`). `MatchEvidence` retains the versioned PURL; byte-identical replay of
the vulnerable SBOM did not duplicate the target exposure or case.

**Success criteria (target-scoped):** `GHSA-6c3j-c64m-qhgq` / `CVE-2019-11358`
is present on `jquery@3.3.9`, absent as an active finding on `jquery@3.4.0`,
alias and PURL range evidence persist, and replay stays clean for that advisory.
Live OSV may also return additional applicable jQuery GHSA records (for example
`GHSA-gxr4-xjj5-5px2` and `GHSA-jpcq-cgw6-v4j6`); those are informational
fan-out notes only and do **not** downgrade this drill from PASS when the
target checks above hold. The automated acceptance test scopes assertions to
the target GHSA rather than unqualified total active exposure counts.
...
### M2 closeout drill C: dump/restore + MinIO sync and digest

This drill exercises the production-shaped **host-run** object-storage overlay on
stock root `docker compose` (`postgres`, `valkey`, `minio`). It is a logical
backup rehearsal, not WAL PITR — the checked-in Postgres image sets
`archive_mode=off`.

1. `docker compose up -d postgres valkey minio`
2. `set -a && source deploy/env/host-staging-object-storage.env && set +a`
3. `uv run python scripts/object_storage_smoke.py` (expect `digest_ok: true`)
4. Confirm bucket versioning on `vulnops-snapshots` and `vulnops-snapshots-backup`
5. `aws s3 sync s3://vulnops-snapshots s3://vulnops-snapshots-backup --endpoint-url "${OBJECT_STORAGE_ENDPOINT}"`
6. Verify digests with `sha256sum` and `verify_sbom_object_digest` (see
   [backup-restore.md](backup-restore.md))
7. `pg_dump -Fc` from the running compose Postgres → restore into isolated DB
   `vulnops_restore_test` → compare row counts for key tables and full schema

Full commands and recovery ordering live in [backup-restore.md](backup-restore.md).
...
| 2026-09-13 | 830505d0 | `M2-E2E-MATCH-001`: authenticated Host Inventory → host-bound CycloneDX PURL → live OSV range match → exposure/case → content-addressed replay | Keycloak 26.3 + Postgres + Valkey + MinIO + host API/worker/orchestrator | PASS (target-scoped) | The inputs contained no vulnerability identifiers. Target `GHSA-6c3j-c64m-qhgq` / `CVE-2019-11358`: one active exposure and one linked case on `jquery@3.3.9`, zero active target exposures on `jquery@3.4.0`, persisted `pkg:npm/jquery` range `1.1.4`–`3.4.0`, and replay-safe SBOM identity. Two hosts, two source snapshots, IP aliases, component→asset bindings, and queue drain were verified. Note: live OSV also returned two other applicable jQuery GHSA records (total active counts 3 on vulnerable / 2 on fixed host); non-blocking fan-out only. |
| 2026-09-13 | 4235abf | OpenVAS/DD live operator: verified finding → OIDC-authenticated asset/SBOM match → confirmed exposure/case → replay | Keycloak 26.3 + DefectDojo 2.51 + Postgres + Valkey + MinIO + host API/ingestion/orchestrator/poller | PASS (target path) | Public OpenVAS XML imported as test `6`; finding `43` was verified and enriched with `CVE-2019-11358`, `pkg:npm/jquery@3.3.9`, version `3.3.9`, and host `m2-vulnerable-web-01`. The bridge normalized DD `vulnerability_ids`/`finding_meta`, persisted Greenbone/OpenVAS provenance, and created exposure `exp_1586f14156bc` plus case `case_c0903e4c3cbe` (`CASE-DB3719EA`). Anonymous API access was `401`; replay left target snapshot/event/exposure/case at `1/1/1/1`. Jira delivery, enterprise IdP, shared adopter staging, and production WAL PITR remain open. |
| 2026-09-13 | 9f1ef35 | M2 closeout drill C: PostgreSQL dump/restore + MinIO versioned sync + SBOM digest verification | Stock compose Postgres 16 + Valkey + MinIO; host `OBJECT_STORAGE_*` from `deploy/env/host-staging-object-storage.env` | PASS (rehearsal; not WAL PITR) | `object_storage_smoke.py` returned `digest_ok: true` for `s3://vulnops-snapshots/sbom/org-smoke/556a2835e9feaf8d5b899871f9e24aa551423885663b261e2f582dfca1171224.json`. Versioning enabled on `vulnops-snapshots` and `vulnops-snapshots-backup`; `aws s3 sync` copied 2 SBOM objects (including an org-demo SBOM-shaped key). Digests checked with `sha256sum` and `verify_sbom_object_digest`. `pg_dump -Fc` restored into `vulnops_restore_test` with matching row counts for `source_snapshots`, `exposures`, `outbox_events`, `remediation_cases`, `vulnerabilities`, `alembic_version`, and all 22/22 tables. Compose Postgres has `archive_mode=off` — dump/restore only. Host `postgresql-client` 17 vs server 16 ignored `transaction_timeout` on restore; data verified OK. |
