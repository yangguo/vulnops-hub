...
def create_app() -> FastAPI:
    ...
    try:
        from vulnops.api.assets import router as assets_router
    ...
    try:
        ...
        app.include_router(
            exposures_router, prefix="/api/v1", dependencies=[Depends(get_principal)]
        )
    except Exception as e:
        logger.warning("exposures router not loaded: %s", e)

    try:
        from vulnops.api.evidence import router as evidence_router

        app.include_router(evidence_router, prefix="/api/v1", dependencies=[Depends(get_principal)])
    except Exception as e:
        logger.warning("evidence router not loaded: %s", e)
