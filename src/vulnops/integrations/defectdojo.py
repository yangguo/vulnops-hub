...
class DefectDojoBridge:
    ...
    def _extract_scan_metadata(self, raw: dict[str, Any]) -> dict[str, Any]:
        ...
        test_type = test_type or self._dd_label(
            test_object.get("test_type_name"), "name", "test_type_name", "scan_type"
        )
        test_type = test_type or self._dd_label(
            test_object.get("test_type"), "name", "test_type_name", "scan_type"
        )
        ...
        scan_type = scan_type or self._dd_label(test_object.get("scan_type"), "name", "scan_type")
        ...
        test_id = test_object.get("id") or test_object.get("test_id")
    ...
    def _normalize_scanner(*values: str | None) -> str:
        ...
        if any(token in value.lower() for value in labels for token in ("greenbone", "openvas")):
