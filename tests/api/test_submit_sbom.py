...
def test_submit_cyclonedx_sbom_accepted():
    ...
    bom = {
        ...
        "components": [
            {"type": "library", "name": "urllib3", "version": "1.26.18", "purl": "pkg:pypi/urllib3@1.26.18"}
        ],
    }
    resp = client.post(
...
def test_duplicate_sbom_upload_is_idempotent():
    ...
    payload = json.dumps(bom, sort_keys=True).encode()
    ...
    assert r1.json().get("content_sha256") == r2.json().get("content_sha256") or r1.json().get("id") == r2.json().get("id")
        ...
        "packages": [
            {
                ...
                "versionInfo": "3.0.2",
                "externalRefs": [{"referenceCategory": "PACKAGE-MANAGER", "referenceType": "purl", "referenceLocator": "pkg:deb/debian/openssl@3.0.2"}],
            }
        ],
    }
