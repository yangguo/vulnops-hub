...
def test_defectdojo_to_closed_case_e2e():
    ...
    session.add(
        AssetAlias(
            asset_id="ast_e2e_01", namespace="cmdb", value="CI-009871", organization_id="org1"
        )
    )
    session.commit()
    ...
    advisory = {
        ...
        "affected": [
            {
                "package": {"ecosystem": "Debian", "purl": "pkg:deb/debian/openssl"},
                "ranges": [
    ...
    exposure = matcher.evaluate(
        comp,
        advisory,
        asset_context={"criticality": "critical", "internet_exposure": "external"},
    ...
    risk_input = PolicyInput(
        ...
        internet_exposure="external",
        match_confidence=exposure.confidence,
        match_class=exposure.match_class,
    )
    ...
    case = case_svc.create_case(
        organization_id="org1",
        title="Fix CVE-2026-12345 on payments-api-3",
        owner_team="secops",
    ...
    verification = case_svc.verify(
        case.id,
        method="wazuh_inventory",
        evidence_ids=[wazuh_result.evidence_ref],
    ...
    case_svc.reopen_on_evidence(
        case.id, evidence_id="ev_new", reason="new scanner confirmed detection"
    )
